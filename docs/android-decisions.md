...
Ghostex Android now installs beside upstream Termux by using the runtime package id `com.ghostx`. The Java namespace remains `com.termux` for upstream sync, while the build generates local bootstrap archives patched from `/data/data/com.termux/files/usr` to `/data/data/com.ghostx/files/usr`. The package id is intentionally the same byte length as `com.termux`; a longer branded id requires rebuilt Ghostex bootstrap and package artifacts.
...
- For the side-by-side `com.ghostx` debug app, phone setup now downloads the official Termux OpenSSH/sshpass packages, rewrites their same-length `/data/data/com.termux` payload and script paths to `/data/data/com.ghostx`, rebuilds local debs, and installs those patched archives. This keeps testing unblocked before a dedicated Ghostex Android package repository exists.
