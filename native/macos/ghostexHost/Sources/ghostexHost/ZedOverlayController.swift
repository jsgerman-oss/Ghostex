...
final class ZedOverlayController: NSObject {
  ...
  private static let logger = Logger(subsystem: "com.madda.ghostex.host", category: "zed-overlay")
  ...
  */
  init(
    ...
    /**
     ...
     visible frame before tucking ghostex so launch restore keeps the user's
  ...
  func setCompanionApplicationBundleIdentifiers(_ identifiers: Set<String>) {
    /**
     ...
     browser activation as a signal to move ghostex offscreen.
     */
  ...
  private func handleActivatedApplication(
    ...
    /**
     ...
     appears after clicking between Canary, ghostex/sidebar, Zed, and the
    ...
    if let application, isTargetApplication(application) {
        ...
        !shouldIgnoreTargetActivationForDockSurfacing
      {
        /**
         ...
         tuck ghostex and any companion browser window away while leaving
        ...
        */
        ...
        /**
         ...
         step before re-keying ghostex. Only that in-flight ordering activation
         should skip the normal "click IDE to hide ghostex" behavior; later direct
         user clicks on Zed must tuck ghostex again.
         */
        ...
        /**
         ...
         opening the project. That programmatic activation must not hide ghostex;
    ...
    if let application, application.processIdentifier == ProcessInfo.processInfo.processIdentifier {
      ...
      updateButtonPosition()
      BrowserOverlayRestoreReproLog.append(
        "zedOverlay.activation.ghostexBranch",
        [
          "application": applicationSummary(application),
          "isSurfacingTargetBehindWindow": isSurfacingTargetBehindWindow,
          "isUserInitiated": userInitiatedActivation,
    ...
    /**
     CDXC:ZedOverlay 2026-04-26-07:19
     Activating unrelated apps should not move the attached ghostex window
     offscreen. Drop ghostex back to normal ordering and hide only the native
  ...
  private func toggleWindow() {
    ...
    if isWindowVisibleInAttachment {
      /**
       ...
       ghostex. If the configured IDE is already running, raise/activate it before
       tucking ghostex. If it is not running, launch it for the current workspace
  ...
  private func showWindow() {
    ...
    /**
     ...
     Restoring ghostex from the native Zed overlay button should also transfer
     macOS focus to ghostex so keyboard input goes to the terminal/workarea
  ...
  private func refocusWindowAfterProgrammaticWorkspaceOpen() {
    /**
     ...
     must return to ghostex so the workspace stays keyboard-ready instead of
  ...
  private func surfaceTargetApplicationBehindWindow() {
    ...
    if targetApplication.isActive {
      /**
       ...
       Alt-tab into ghostex should not pay the IDE surfacing delay when the
       attached IDE is already active. Preserve the successful ordering flow
       for inactive targets, but bring ghostex back immediately from the hidden
    ...
    /**
     ...
     alt-tab activation shows ghostex only after the refocus pass. Logs at 02:33
     showed Zed activating on the first retry while ghostex waited another 365ms
  ...
  private func raiseTargetApplicationWindow(_ application: NSRunningApplication) -> Bool {
    /**
     ...
     app activation after the current ghostex activation notification settles,
     then re-key ghostex so the IDE becomes the normal-level window directly
  ...
  private func updateButtonTitle() {
    /**
     ...
     keeps ghostex-named internals for existing settings and diagnostics.
     */
    toggleButton?.title =
  ...
  private func makeButtonPanel() -> NSPanel {
    ...
    /**
     ...
     The native button must remain clickable without activating ghostex and must
    ...
    /**
     ...
     ghostex toggle. Detach remains a settings-level attach-state action, not
  ...
  private func windowFrame(attachedTo frame: TargetWindowFrame) -> NSRect {
    /**
     ...
     Attached ghostex stays resizable, but its maximum attached size is inset
     ...
     Attaching to an IDE should preserve the current ghostex size whenever that
