...
final class BrowserOverlayController {
  ...
  private static let logger = Logger(subsystem: "com.madda.ghostex.host", category: "browser-overlay")
  ...
  /**
   ...
   Chrome Canary, move that browser window over the current ghostex workarea
   instead of the sidebar chrome, and temporarily let Chrome be the foreground
   app so it can sit above attached ghostex without Terminal or webview browser
   fallbacks.
   */
  init(
    window: NSWindow,
  ...
  func moveBrowserOffscreen() {
    ...
    /**
     ...
     When Zed is clicked, tuck Chrome Canary away with ghostex rather than
     closing or minimizing it. Store the visible workarea-sized frame so the
     native ghostex button can restore the browser exactly into the workarea.
     CDXC:BrowserOverlay 2026-04-26-09:11
     Restore should preserve the last attached browser state. If Canary was
     merely running but had not been shown above ghostex, do not capture and
     restore it when the native ghostex button brings the app forward again.
     */
  ...
  func showRunningChromeCanary() {
    ...
    /**
     ...
     over the ghostex workarea, not open another URL or fall back to a webview.
     */
    setCompanionBrowserActive(true)
    window?.level = .normal
    if hasNoChromeCanaryWindows(for: application) {
      /**
       ...
       window over the ghostex workarea through the same AX placement path.
       */
  ...
  func markBrowserNoLongerShownInAttachment(reason: String) {
    /**
     ...
     If the user clicks back into ghostex/sidebar while Canary had been shown
...
enum BrowserOverlayRestoreReproLog {
  private static let logger = Logger(
    subsystem: "com.madda.ghostex.host", category: "browser-overlay-repro")
  ...
  /**
   ...
   tracked Canary window was actually shown when ghostex was tucked. Keep these
